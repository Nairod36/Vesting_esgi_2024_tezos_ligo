...
let start (s : storage) : ret =
  let admin = Tezos.get_sender() in
  if s.is_started then
    failwith Errors.contract_already_started
  else if admin = Some(s.admin) then
    let now = Tezos.get_now() in
    let updated_storage = {
      s with 
      vesting_config = {s.vesting_config with start_time = add_seconds(now, s.vesting_config.freeze_duration)};
      is_started = true
    } in
    ([], updated_storage)
  else
    ...
    | Some(detail) ->
      ...
      if now < freeze_end_time then
        ...
        if claimable_amount <= 0n then
          ...
          let (ops, new_storage) = transfer (VestingFA2.TZIP12.transfer transfer_ops) s in
          let updated_detail = {detail with claimed_amount = detail.claimed_amount + claimable_amount} in
          let updated_beneficiaries = Big_map.update addr (Some(updated_detail)) new_storage.beneficiaries in
          (ops, {new_storage with beneficiaries = updated_beneficiaries})
...
let kill (s : storage) : ret =
  let admin = Tezos.get_sender() in
  match admin with
  | Some(admin_address) when admin_address = s.admin ->
    let vesting_end_time = add_seconds(s.vesting_config.start_time, s.vesting_config.vesting_duration) in
    let now = Tezos.get_now() in
    if now < vesting_end_time then
