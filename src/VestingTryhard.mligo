 #import "../.ligo/source/i/ligo__s__fa__1.2.0__ffffffff/lib/main.mligo" "FA2"
  ...
  type extension = {
    ...
    beneficiary_list: address list;  // List of all beneficiary addresses
...
let kill (storage : storage) : ret =
  ...
  if sender <> storage.extension.admin then
    ...
    let now = Tezos.get_now() in
    let (ops, updated_beneficiaries) = List.fold (fun (acc_ops, acc_beneficiaries) beneficiary_address ->
      match Big_map.find_opt beneficiary_address acc_beneficiaries with
      | None -> (acc_ops, acc_beneficiaries)  // No action if the beneficiary is not found
      | Some(details) -> // POURQUOI IL EST TT LE TEMPS VIDEEEEE
        ...
        let vested_tokens = 
          if elapsed_time >= total_vesting_time then
            details.promised_amount  // All promised tokens can be claimed after the vesting period
          else
            let progress = (details.promised_amount * elapsed_time) / total_vesting_time in
            progress  // Calculate proportional amount of tokens that can be claimed
        in
        ...
        if to_claim > 0 then
          ...
          let updated_details = { promised_amount = details.promised_amount; claimed_amount = vested_tokens } in
          let new_beneficiaries = Big_map.update beneficiary_address (Some(updated_details)) acc_beneficiaries in
          (transfer_op :: acc_ops, new_beneficiaries)
        else (acc_ops, acc_beneficiaries)
    ) ([], storage.extension.beneficiaries) storage.extension.beneficiary_list in
    (ops, { storage with extension = { storage.extension with beneficiaries = updated_beneficiaries; is_started = storage.extension.is_started; admin = storage.extension.admin; fa2_token_address = storage.extension.fa2_token_address; token_id = storage.extension.token_id; vesting_config = storage.extension.vesting_config } })
